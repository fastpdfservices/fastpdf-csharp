...
namespace FastPDFService.Models
{
    ...
    public class ImageFile
    {
        ...
        public override int GetHashCode()
        {
            ...
            {
                int hash1 = 17;
                hash1 = hash1 * 23 + (Format?.GetHashCode() ?? 0);
                hash1 = hash1 * 23 + (Uri?.GetHashCode() ?? 0);
                hash1 = hash1 * 23 + (Description?.GetHashCode() ?? 0);
                hash1 = hash1 * 23 + (ImageFileData?.GetHashCode() ?? 0);
                hash1 = hash1 * 23 + Id.GetHashCode();
                hash1 = hash1 * 23 + Number.GetHashCode();
                hash1 = hash1 * 23 + Timestamp.GetHashCode();

                int hash2 = 17;
                hash2 = hash2 * 23 + (TemplateId?.GetHashCode() ?? 0);
                hash2 = hash2 * 23 + (Filename?.GetHashCode() ?? 0);
        ...
        public override string ToString()
        {
            var templateFileData = ImageFileData != null 
                ? BitConverter.ToString(ImageFileData).Substring(0, Math.Min(20, ImageFileData.Length)) + "..." 
                : "null";
